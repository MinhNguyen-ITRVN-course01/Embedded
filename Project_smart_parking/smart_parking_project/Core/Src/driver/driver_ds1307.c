...
// Define a macro to check if a year is a leap year
#define CHECK_LEAP_YEAR(x) \
    ((x % 4 == 0) ? true : false)

// Define a macro to check if the date of February is valid
#define CHECK_DATE_OF_FEB(x) \
    ((x == 28 || x == 29) ? true : false)

// Define a macro to check if the date of a month with 30 days is valid
#define CHECK_DATE_OF_MONTH_30(x) \
    ((x <= 30) ? true : false)
...
static bool check_time_range_ds1307(set_ds1307_t *ptr_check)
{
    ...
    || ((ptr_check->set_ds1307_month == 4 || ptr_check->set_ds1307_month == 6 || ptr_check->set_ds1307_month == 9 || ptr_check->set_ds1307_month == 11) && CHECK_DATE_OF_MONTH_30(!ptr_check->set_ds1307_date))
    || (CHECK_LEAP_YEAR(ptr_check->set_ds1307_year) == true && ptr_check->set_ds1307_month == 2 && CHECK_DATE_OF_FEB(!ptr_check->set_ds1307_date))) 
    return false;
    else return true;
}
...
void ds1307_gettime(ds1307_t *ptr_gettime)
{
    ...
    ds1307_cen = ds1307_get_reg_byte(DS1307_REG_CENT) * 100;
...
{
    if (check_time_range_ds1307(ptr_settime) == true)
    {
